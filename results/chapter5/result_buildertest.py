...
def build_chart(chart, data_set, chart_type = 'plot', ticks = False, show = True, ylim = False, xlim = False):
    pylab.ioff()
    fig_width_pt = 350
    ...
    if chart_type == 'plot':
        ...
        ax.legend((one[0], two[0]), chart['LEGEND'], bbox_to_anchor=(0.0, 1.02, 1.0, 0.102), loc=3, ncol=4, mode='expand', borderaxespad=0.0)
        ...
        setFigLinesBW(fig)
        fig.grid(False)
        fig.legend(bbox_to_anchor=(0.0, 1.02, 1.0, 0.102), loc=3, ncol=4, mode='expand', borderaxespad=0.0)
        if xlim:
            pylab.xlim(chart['XMIN'], chart['XMAX'])
        else:
...
def sens_results(n = 110):
    ...
    with open(home + table_out + 'welfare_sens.txt', 'w') as f:
        f.write(tab.to_latex(float_format='{:,.2f}'.format, columns=['Mean',
         'Min',
         'Q1',
         'Q3',
         'Max']))
    ...
    with open(home + table_out + 'welfareI_sens.txt', 'w') as f:
        f.write(tab.to_latex(float_format='{:,.3f}'.format, columns=['Mean',
         'Min',
         'Q1',
         'Q3',
         'Max']))
